...
| P2 | Local quantum circuit planning, simulation, observables, and sampling | Implemented bounded foundation | `quantum.*` validates local circuit maps including finite numeric `rx`/`ry`/`rz` rotations, emits/writes deterministic OpenQASM 3.0, calculates full-basis state-vector probabilities, evaluates one fixed-length Pauli-product expectation, and returns explicit-seed reproducible counts for up to 12 qubits. It does not bind symbolic parameters, use a hidden seed/noise/collapse state, evaluate Pauli sums/Hamiltonians, execute, submit, authenticate to, or select a quantum provider/QPU. |
