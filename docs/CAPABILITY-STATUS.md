...
| P2 | Local quantum circuit planning and simulation | Implemented bounded foundation | `quantum.*` validates local circuit maps, emits/writes deterministic OpenQASM 3.0, and calculates deterministic full-basis state-vector probabilities for up to 12 qubits. It does not sample counts, model noise, execute, submit, authenticate to, or select a quantum provider/QPU. |
