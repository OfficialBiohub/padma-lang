...
| Client documents v1 | Bounded | Validates and renders local `quote` and `invoice-draft` Markdown; may write a reviewed `.md` file inside a project. | `client.document_markdown`, `client.document_summary`, `client.write_document`; scope-of-work, delivery checklist, and case-study schemas are **not yet implemented**. |
...
1. **P0 local record schemas** for student/family/small-business work is the next selected item, built on existing safe table/report primitives.
2. Then expand **P0 freelancer workflows** with one bounded document schema at a time, starting with scope-of-work and delivery checklist.
3. Continue P1 and P2 items only after their dependencies and safety contracts are complete.
