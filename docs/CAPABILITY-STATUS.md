...
| P2 | Local quantum circuit planning, simulation, and observables | Implemented bounded foundation | `quantum.*` validates local circuit maps, emits/writes deterministic OpenQASM 3.0, calculates full-basis state-vector probabilities, and evaluates one fixed-length Pauli-product expectation for up to 12 qubits. It does not sample counts, model noise, evaluate Pauli sums/Hamiltonians, execute, submit, authenticate to, or select a quantum provider/QPU. |
