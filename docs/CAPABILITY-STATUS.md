...
| Website/backend examples | Safe HTML-file creation, local response-envelope construction, explicit in-memory route matching with `server.route_response`, and a capability-gated fixed `127.0.0.1:8080` route server reading project-local `server-routes.json`. | A full web framework, public production server, user auth service, database-backed API, managed deployment, or SaaS backend. |
