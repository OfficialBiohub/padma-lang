...
| Client documents v1 | Bounded | Validates and renders local `quote`, `invoice-draft`, and scope-of-work Markdown; may write a reviewed `.md` file inside a project. | `client.document_markdown`, `client.document_summary`, `client.write_document`, `client.scope_markdown`, `client.scope_summary`, and `client.write_scope`; delivery checklist and case-study schemas are **not yet implemented**. |
...
| P0 | Freelancer client workflow expansion | Partial | Quote/invoice-draft and scope-of-work are implemented foundations. Add separately bounded delivery checklist, portfolio case-study, client-task, and reusable report schemas. |
...
1. Expand **P0 freelancer workflows** with the next bounded document schema: delivery checklist.
