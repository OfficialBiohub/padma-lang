...
| Client documents v1 | Bounded | Validates and renders local `quote`, `invoice-draft`, scope-of-work, and delivery-checklist Markdown; may write a reviewed `.md` file inside a project. | `client.document_markdown`, `client.document_summary`, `client.write_document`, `client.scope_*`, and `client.delivery_*`; portfolio case-study schemas are **not yet implemented**. |
