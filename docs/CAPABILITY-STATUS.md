...
| Website/backend examples | Safe HTML-file creation, local response-envelope construction, explicit in-memory route matching with `server.route_response`, a capability-gated fixed `127.0.0.1:8080` route server reading project-local `server-routes.json`, and an M41 local SQLite student/product data layer example. | A full web framework, database-backed HTTP route dispatch, public production server, user auth service, managed deployment, or SaaS backend. |
...
| P2 | Local backend route server | Implemented bounded foundation | `server.route_response` provides deterministic route semantics, while `padma serve .` with `server:local` loads only project-local `server-routes.json`, binds fixed `127.0.0.1:8080`, enforces bounded HTTP parsing, writes JSON responses, and supports terminal shutdown. It has no public bind, database, authentication, payment, deployment, callback, or arbitrary process authority. |
