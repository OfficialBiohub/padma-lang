...
| P1083 | Local quantum circuit planning safety | A local quantum circuit has missing/unknown/provider fields, malformed operations or measurements, unsupported gates, invalid/duplicate indexes, an incomplete measurement map, or oversized OpenQASM output. It only generates local circuit text; provider/QPU/simulator/credential/network/process execution and algorithm-performance claims are unavailable. |
