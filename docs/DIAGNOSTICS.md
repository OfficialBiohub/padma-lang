...
| P1083 | Local quantum circuit planning safety | A local quantum circuit has missing/unknown/provider fields, malformed operations or measurements, unsupported gates, invalid/duplicate indexes, malformed/missing/extra/non-finite/oversized rotation angles, an incomplete measurement map, or oversized OpenQASM output. It only generates/simulates bounded local circuit data; provider/QPU/credential/network/process execution and algorithm-performance claims are unavailable. |
| P1084 | Local quantum simulator safety | A local simulation exceeds the 12-qubit state-vector limit or fails a finite normalized-state/probability invariant. It only returns deterministic local probability data; sampling/counts, QPU/provider/credential/network/process execution, noise modeling, and performance claims are unavailable. |
| P1085 | Local Pauli observable safety | A Pauli observable is empty, non-ASCII, wrong-length, contains a character outside `I`/`X`/`Y`/`Z`, or produces a non-real/out-of-range local expectation invariant. It only evaluates one local Pauli product; sampling, Pauli sums/Hamiltonians, optimisation, QPU/provider/credential/network/process execution, noise, and performance claims are unavailable. |
| P1086 | Local quantum sampling safety | A sampling request is missing/unknown, has absent/non-whole/out-of-range `shots` or `seed`, local probability data is malformed, or the count-total invariant fails. It returns only explicit-seed bounded local pseudo-random counts; no hidden seed, collapse state, provider/QPU/credential/network/process action exists. |
| P1087 | Local Pauli Hamiltonian safety | A Hamiltonian map/term is missing, unknown, empty, duplicated, wrong-length, non-Pauli, zero/non-finite/out-of-range coefficient, exceeds local term/numeric limits, or fails an energy invariant. It returns deterministic local energy only; optimiser, symbolic coefficient, VQE/QAOA/QML, provider/QPU/credential/network/process action are unavailable. |
| P1088 | Local optimisation safety | A quadratic objective/settings map is missing, unknown, malformed, non-finite, mismatched, outside a local vector/bounds/weight/epsilon/learning-rate limit, fails the strict epsilon-interior condition, or breaks a local numerical invariant. It returns only deterministic in-memory value/gradient/one-step proposal data; no loop, mutation, callback, model training, VQE/QAOA/QML, provider/QPU/credential/network/process action is available. |
| P1089 | Local OpenQASM subset-assessment safety | An assessment source is empty, non-text, non-ASCII, oversized, or not exactly equal to the bounded local renderer output for its validated circuit. Comments, altered declarations, gates, rotation formatting, whitespace, or measurements are rejected rather than parsed/imported. It returns local metadata only; source parser/import/execution, file/network/process/provider/QPU/credential action is unavailable. |
| P1090 | Quantum provider readiness assessment safety | A provider-readiness map has missing/unknown/unsupported provider or artifact fields, invalid fingerprint/byte bound, non-public policy note, or any credential/account/job/endpoint/source/action material. It returns only redacted local controls; authentication, provider SDK, backend selection, cost/quota lookup, job/task submission, polling, cancellation, provenance, QPU/network/process action are unavailable. |
| P1091 | Local backend route safety | A route request or route list is missing, unknown, malformed, duplicated, non-finite, oversized, uses an unsupported method/status, contains an unsafe path, or includes external/action fields. It returns only a deterministic in-memory JSON response envelope; no callback, database, file, network, process, credential, browser, payment, or deployment action exists. |
| P1092 | Typed local data safety | A fixed student/product record has missing, duplicate, unknown, empty, oversized, non-finite, or out-of-range fields, or the typed SQLite operation uses an invalid key/limit. It only stores validated local JSON through `database:sqlite`; ORM, arbitrary SQL, public HTTP dispatch, credential, payment, cloud, and deployment actions are unavailable. |
| P1093 | Local database-route safety | `server-data-routes.json` is missing/inexact/malformed, uses an unsafe/non-local/non-regular/missing database, mixes with static routes, has unsafe/duplicate paths, or maps to an unsupported collection. The local server reads only fixed typed `student`/`product` lists with `GET` and `sqlite3 -readonly`; arbitrary SQL, writes, query inputs, auth, payment, cloud, public binding, and deployment are unavailable. |
