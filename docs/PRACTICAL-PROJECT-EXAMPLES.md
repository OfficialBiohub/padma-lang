...
| Local quantum circuit planning | `examples/local-quantum-planning` | Bounded circuit map to deterministic OpenQASM 3.0 and local exact probability data | It cannot sample counts, use a QPU/provider, read credentials, submit a job, access a network, or start a process. |
