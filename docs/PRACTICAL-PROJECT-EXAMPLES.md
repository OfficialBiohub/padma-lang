...
| Local quantum circuit planning | `examples/local-quantum-planning` | Bounded circuit map to deterministic OpenQASM 3.0, local exact probability data, and single Pauli-product expectations | It cannot sample counts, evaluate a Hamiltonian/algorithm, use a QPU/provider, read credentials, submit a job, access a network, or start a process. |
