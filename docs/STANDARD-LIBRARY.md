...
The strict map accepts only a `1..20` qubit count, supported gate maps (`h`, `x`, `z`, `s`, `t`, `rx`, `ry`, `rz`, `cx`, `superposition`, `entangle-linear`), and a complete unique measurement mapping. Each rotation needs one finite numeric `angle` in radians with absolute value at most `1_000_000`; no symbolic parameter exists yet. The simulator and Pauli observable evaluator further limit themselves to 12 qubits. `expectation_pauli` accepts exactly one ASCII Pauli string with one `I`, `X`, `Y`, or `Z` character per qubit; its leftmost character applies to the highest qubit index. There is no sampled count, collapse/partial measurement, QPU/provider/device selection, cloud transport, credential use, Pauli sum/Hamiltonian, QAOA/QML/Grover implementation, noise mitigation, or real-device result data. See [`LOCAL-QUANTUM-PLANNING.md`](LOCAL-QUANTUM-PLANNING.md), [`LOCAL-QUANTUM-SIMULATOR.md`](LOCAL-QUANTUM-SIMULATOR.md), [`LOCAL-QUANTUM-OBSERVABLES.md`](LOCAL-QUANTUM-OBSERVABLES.md), and [`LOCAL-QUANTUM-ROTATIONS.md`](LOCAL-QUANTUM-ROTATIONS.md).
