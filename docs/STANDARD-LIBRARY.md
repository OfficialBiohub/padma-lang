...
| `quantum.assess_openqasm3(circuit, source)` | Checks only that bounded in-memory ASCII `source` exactly matches Padma’s deterministic renderer for `circuit`, then returns stable local artifact metadata. No capability is required. |
| `quantum.provider_readiness(request)` | Validates a provider label, reviewed OpenQASM artifact fingerprint, and redacted public policy note; returns only no-side-effect readiness controls. No capability is required. |
...
`assess_openqasm3` is intentionally an exact renderer-consistency assessment, not a general OpenQASM parser or round-trip importer: source must be non-empty ASCII, no more than `1,048,576` bytes, and byte-for-byte equal to `openqasm3(circuit)`. Comments, altered declarations, unsupported gates, or changed measurement assignments are rejected even when another OpenQASM implementation might accept them. It has no file read/write, import, execution, provider/QPU/credential/network/process authority. See [`LOCAL-QUANTUM-INTERCHANGE.md`](LOCAL-QUANTUM-INTERCHANGE.md).

`provider_readiness` accepts exactly `provider` (`ibm-quantum`, `aws-braket`, or `other-reviewed`), bounded `artifact` fingerprint metadata, and a public `policyNote` that is accepted but not returned. It is a local assessment only: no token, saved account, endpoint, backend, pricing/quota, job/task, source text, provider SDK, authentication, submission, polling, cancellation, provenance, QPU, network, or process action exists. See [`QUANTUM-PROVIDER-READINESS.md`](QUANTUM-PROVIDER-READINESS.md).

## Bounded local optimisation primitives

| API | Result and boundary |
|---|---|
| `optimize.quadratic_value(objective)` | Returns the deterministic scalar value of one strict finite separable weighted quadratic map. No capability is required. |
| `optimize.finite_difference_gradient(objective, epsilon)` | Returns a centered finite-difference gradient for an epsilon-interior parameter vector. No capability is required. |
| `optimize.projected_gradient_step(objective, settings)` | Returns one non-mutating bounds-clamped proposal with before/after values; `settings` is exactly `{learningRate, epsilon}`. No capability is required. |
