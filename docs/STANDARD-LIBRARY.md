...
The strict map accepts only a `1..20` qubit count, supported gate maps (`h`, `x`, `z`, `s`, `t`, `cx`, `superposition`, `entangle-linear`), and a complete unique measurement mapping. The simulator further limits itself to 12 qubits and returns probability values only. There is no sampled count, collapse/partial measurement, QPU/provider/device selection, cloud transport, credential use, QAOA/QML/Grover implementation, noise mitigation, or real-device result data. See [`LOCAL-QUANTUM-PLANNING.md`](LOCAL-QUANTUM-PLANNING.md) and [`LOCAL-QUANTUM-SIMULATOR.md`](LOCAL-QUANTUM-SIMULATOR.md).
