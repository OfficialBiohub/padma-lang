...
Projects may further declare `browser = ["plan", "confirm-plan", "handoff"]` and use `padma browser handoff [dir]` to hand exactly one digest-bound reviewed HTTPS URL to the fixed local `termux-open-url` command only after a person types the foreground confirmation `OPEN`. This narrow handoff starts one local opener process; it does not attach to or control a browser, read a profile/cookie/credential, pass headers/scripts/selectors/proxy settings, perform DNS/network activity itself, or permit login, CAPTCHA bypass, form submission, posting, upload/download, account change, purchase, or payment.

Projects may independently declare `browser = ["audit"]` and add `padma-browser-audit.toml` to opt into a bounded, project-local redacted handoff audit. The audit authority does not imply `browser:handoff` and cannot invoke a process. Its fixed JSONL schema excludes raw URLs/query strings, terminal input, cookies, credentials, profile data, headers, page content, and browser output; unsafe existing records, symlinks, path traversal, and write failures are rejected with `P1064`.

Projects may separately declare `browser = ["plan", "draft"]` and use `padma browser draft inspect|plan` to validate a strict `padma-browser-draft.toml` review descriptor. It binds bounded text, optional project-relative attachment metadata, and one reviewed plan digest/index; it neither reads the attachment nor starts a browser, resolves DNS, connects to a network, fills/submits a form, posts, uploads/downloads, accesses credentials/cookies/profiles, injects JavaScript, or executes generated output. Login, CAPTCHA, form completion, post/message, upload/download, account change, purchase, and payment stay `user-takeover-required` in the person’s visible browser.

Projects may separately declare `browser = ["plan", "takeover"]` and use `padma browser takeover inspect|plan` to validate a strict `padma-browser-takeover.toml` visible user-takeover checklist. It binds one reviewed digest/index to a fixed sensitive-action label and emits no approval or execution authority. It does not launch/drive a browser, resolve DNS, use network/processes, inspect a page, read credentials/cookies/profiles, fill or submit a form, post, upload/download, change an account, purchase, pay, inject JavaScript, execute generated output, or collect a user decision.
...
| Android Browser Handoff | Existing `browser = ["plan", "confirm-plan", "handoff"]` | Foreground `OPEN` confirmation plus one fixed local `termux-open-url` call containing one digest-bound reviewed HTTPS URL; no browser control, profile/session read, generic process arguments, login, form, post, payment, upload, or download. | [Android Browser Handoff](ANDROID-BROWSER-HANDOFF.md) |
| Android Browser Handoff audit | Existing `browser = ["audit"]` plus optional handoff grant | Opt-in bounded project-local fixed-field audit only; no raw URL/query/input/header/cookie/credential/profile/page data, no browser/network/process action, and no implicit handoff authority. | [Android Browser Handoff](ANDROID-BROWSER-HANDOFF.md) |
| Browser interaction draft | Existing `browser = ["plan", "draft"]` | Local digest/index-bound bounded text plus optional attachment metadata only; browser/network/DNS/attachment-read/form/post/upload/payment/credential/generated-output execution remain disabled and all sensitive steps require visible user takeover. | [Browser interaction drafts](BROWSER-DRAFTS.md) |
| Visible browser takeover checklist | Existing `browser = ["plan", "takeover"]` | Local digest/index-bound fixed sensitive-action checklist only; it has no approval, browser, network, page, credential, form, posting, transfer, account, purchase, payment, script, output-execution, or decision-collection authority. | [Visible browser takeover checklist](BROWSER-TAKEOVER.md) |

The browser design intentionally does not widen `network = ["http"]` or create a broad browser privilege. Android Browser Handoff is not `browser:navigate`: it is a fixed, visible operating-system URL handoff. A future browser-control adapter, if separately approved, requires a fresh grant, strict destination verification, isolated browser state, a bounded operation, and visible user confirmation immediately before navigation. The planning contract reserves no implicit upgrade path from `browser:plan` to general browser authority.

The proposed [browser navigation action-adapter design](BROWSER-ACTION-ADAPTER-DESIGN.md) defines further prerequisites for any future browser-control adapter: isolated ephemeral state, revalidation of resolved addresses before each connection, automatic redirect denial, one-time confirmation bound to a canonical plan digest, cancellation, and a redacted local audit event. It remains a review document; no `browser:navigate` capability or browser-control runner exists in this release.
