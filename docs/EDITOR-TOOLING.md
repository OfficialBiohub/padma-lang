...
The initial language-server baseline is now complete in `tooling/padma-lsp`: it uses standard LSP JSON-RPC on stdin/stdout, publishes diagnostics generated from the public CLI-compatible JSON API, converts Padma source positions to LSP UTF-16 columns, returns full-document formatter edits, and offers safe static Bangla-English completion and hover help. Local declaration positions and lexical scope data now come from a compiler-owned parsed analysis API rather than editor-side source-prefix matching. The VS Code extension starts it only through an explicit user command.

Definitions, rename, dynamic symbol completion, incremental range updates, and semantic document analysis remain later work. The extension must not duplicate those semantic decisions.
