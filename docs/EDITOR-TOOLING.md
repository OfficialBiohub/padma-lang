...
The initial language-server baseline is now complete in `tooling/padma-lsp`: it uses standard LSP JSON-RPC on stdin/stdout, publishes diagnostics generated from the public CLI-compatible JSON API, converts Padma source positions to LSP UTF-16 columns, returns full-document formatter edits, and offers safe static Bangla-English completion and hover help. Local declaration positions, lexical scope data, and reference-to-binding relationships come from compiler-owned parsed analysis APIs rather than editor-side source-prefix matching. The VS Code extension starts it only through an explicit user command.

Same-document go-to-definition, dynamic local completion, and conservative local-variable rename are now available. Rename accepts only a valid Padma identifier and creates edits only for the declaration and references tied to one compiler-resolved local binding. It does not rename functions, imports, exports, members, unresolved identifiers, strings, or comments; identically named Bangla or English locals in nested scopes remain isolated. The extension now has a pinned VSIX packaging command, CI artifact validation, and a capability-free sample workspace that exercises the explicit language-server command. Cross-file rename, incremental range updates, and semantic document symbols remain later work. The extension must not duplicate those semantic decisions.
