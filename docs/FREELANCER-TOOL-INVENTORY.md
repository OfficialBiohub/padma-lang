...
| 14 | Local quantum circuit planning | Validates bounded gates, exports deterministic OpenQASM 3.0 circuit text, simulates exact local basis probabilities, and evaluates one Pauli-product expectation up to 12 qubits. | `quantum.*`; `examples/local-quantum-planning` |
...
| P2 | Quantum computing | Implemented local foundations: validated circuit planning/OpenQASM text export, bounded exact state-vector probability simulation, and one Pauli-product expectation evaluator. QPU/cloud submission, provider credentials, Hamiltonian/algorithm libraries, noise mitigation, sampling, and performance claims are intentionally unavailable. |
