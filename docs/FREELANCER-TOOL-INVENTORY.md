...
| 14 | Local quantum circuit planning | Validates bounded gates, exports deterministic OpenQASM 3.0 circuit text, and simulates exact local basis probabilities up to 12 qubits. | `quantum.*`; `examples/local-quantum-planning` |
...
| P2 | Quantum computing | Implemented local foundations: validated circuit planning/OpenQASM text export plus bounded exact state-vector probability simulation. QPU/cloud submission, provider credentials, algorithm libraries, noise mitigation, sampling, and performance claims are intentionally unavailable. |
