...
The current executable core supports UTF-8 identifiers, Bengali digits, numbers, strings with `\\n`, `\\t`, escaped quotes, booleans, null values, variable declaration, assignment, output, arithmetic, comparisons, conditional blocks, and bounded `while` loops. Statements are separated by newlines; blocks use braces.

List literals use square brackets and comma-separated expressions, for example `[১, ২, ৩]`. Lists use safe zero-based non-negative indexes (`items[0]`) and bounded slices (`items[1:3]`, `items[:2]`, `items[2:]`), with `get`, `set`, `push`, `remove`, `len`, and `contains` methods. Slice start is inclusive and end is exclusive; negative, reversed, and out-of-range bounds are rejected. Text-key maps support bracket lookup and `get`, `set`, `len`, and `contains`. Invalid indexes, missing keys, and invalid key types produce localized diagnostics. `for item in collection` / `প্রতি item মধ্যে collection` iterates lists, text characters, and map keys in deterministic order. `range(end)` and `range(start, end)` / `পরিসর(...)` create finite zero-based number lists; every iteration is limited to one million elements.
